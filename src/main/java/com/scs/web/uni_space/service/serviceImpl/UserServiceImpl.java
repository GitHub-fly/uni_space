...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Result signIn(SignDto signDto) {
        User user = null;
        //取得短信验证码
        String verifyCode = redisService.getValue(signDto.getName(), String.class);
        try {
            //判断数据库中是否有该用户
            if (userMapper.selectUserByMobile(signDto.getName()) != null) {
                user = userMapper.selectUserByMobile(signDto.getName());
            } else if (userMapper.selectUserByAccount(signDto.getName()) != null) {
                user = userMapper.selectUserByAccount(signDto.getName());
            } else if (userMapper.selectUserByEmail(signDto.getName()) != null) {
                user = userMapper.selectUserByEmail(signDto.getName());
            } else if (signDto.getVerifyCode() == null) {
                return Result.failure(ResultCode.USER_NOT_EXIST);
            } else {
                //判断短信验证码为空或者是否正确
                if (signDto.getVerifyCode().equals(verifyCode)) {
                    //直接注册
                    user = User.builder()
                    ...
                    try {
                        ...
                        userMapper.insertUser(user.getCode(), signDto.getName(), "4297f44b13955235245b2497399d7a93", user.getAvatar(), user.getCreateTime(), user.getBirthday());
                        //用生成的Base64生成6位字符串的加密串
                        String token = DigestUtils.sha3_256Hex(user.getCode());
                        //token存入redis，时效24小时，客户端拿到token，会变为登录状态
                        redisService.set(user.getCode(), token, 60 * 24L);
        ...
        if (signDto.getPassword() != null) {
            if (user.getPassword().equals(DigestUtils.md5Hex(signDto.getPassword()))) {
                String token = DigestUtils.sha3_256Hex(user.getCode());
                //token存入redis，时效24小时，客户端拿到token，会变为登录状态
                redisService.set(user.getCode(), token, 60 * 24L);
                return Result.success(token);
//                return Result.success(user);
        ...
        //判断用户使用图形验证码登录
        if (signDto.getVerifyCode() != null) {
            if (verifyCode == null) {
                return Result.failure(ResultCode.USER_VERIFY_CODE_NULL);
            } else {
                if (verifyCode.equals(signDto.getVerifyCode())) {
                    return Result.success(user);
                } else {
                    return Result.failure(ResultCode.USER_VERIFY_CODE_ERROR);
                }
    ...
    @Override
    public Result signUp(SignDto signDto) {
        User user = null;
        try {
            //查找数据库中是否有该用户
            user = userMapper.selectUserByMobile(signDto.getName());
                ...
                if (verifyCode.equals(userDto.getVerifyCode())) {
                    if (verifyCode == null) {
                        ...
                        if (user.getPassword().equals(DigestUtils.md5Hex(userDto.getPassword()))) {
                            ...
                            userMapper.updateUserPassword(userDto.getMobile(), DigestUtils.md5Hex(userDto.getPassword()));
                            return Result.success();
                        }
                    }
                } else {
    ...
    public Result updateUserAvatar(UserDto userDto) {
        if (userDto.getAvatar() != null) {
            try {
                userMapper.updateUserAvatar(userDto.getAvatar(), userDto.getId());
                return Result.success();
            } catch (SQLException e) {
                log.error("数据库修改错误");
                return Result.failure(ResultCode.RESULT_CODE_DATA_NONE);
